...
get_client_overdue_projects2 = "SELECT p.project_id FROM Projects p INNER JOIN Current_Team_Project ctp ON p.project_id = ctp.project_id WHERE p.client_id = %s AND deadline < NOW()"
