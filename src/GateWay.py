...
class GateWay(object):
	...
	def add_user(self, user_id, user_password, balance, user_type, user_email, user_address):
		...
		try:
			self.conn.connect()
			self.cursor.execute(insert_user, (user_id, user_password, int(user_type), float(balance), user_email, user_address))
...
# Project Methods
	...
	def get_current_indiv_project(self, project_id):
		if(not ((self.get_project_status(project_id) == "Current") (self.get_project_type(project_id) == "Individual"))):
			return False
		try:
			self.conn.connect()
	...
	def finish_individual_project(self, project_id):
		if(not ((self.get_project_status(project_id) == "Current") and (self.get_project_type(project_id) == "Individual"))):
			return False
